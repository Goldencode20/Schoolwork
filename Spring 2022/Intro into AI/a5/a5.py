...
def calc_entropy(classifications):
    #entropy = 0
    #total_num_of_class = 0
    #total_num_of_class_occurance = 0
    #for ele in range(0, len(classifications)):
    #    total_num_of_class_occurance = classifications.count(classifications[ele])
    #    total_num_of_class = total_num_of_class + classifications[ele]
    #    entropy = (-1 *  (entropy + classifications[ele] + entropy) ) / (entropy + len(classifications)) * math.log(classifications[ele] /len(classifications), 2)
    #return entropy
    entropy = 0
    temp = set(classifications)
    for x in temp:
        true_records = 0
        for y in classifications:
            if x == y:
                true_records += 1
        record_split = true_records/len(classifications)
        entropy += -(record_split * math.log((1/record_split), 2))
    return -1 * entropy
...
def calc_information_gain(parent_classifications, classifications_by_val, val_freqs):
    total_points = 0
    weighted_entropy = 0
    for x in classifications_by_val:
        total_points += len(classifications_by_val.get(x))
    for x in classifications_by_val:
        weighted_entropy += (len(classifications_by_val.get(x))/total_points) * calc_entropy(classifications_by_val.get(x))
...
class Node:
    ...
    def classify(self, point):
        ...
        atrib = point[self.attribute]
        if atrib in self.children.keys():
            return self.children[atrib].classify(point)
        else:
            return self.children[OTHER].classify(point)
...
class KNN_Classifier:
    ...
    def classify(self, point, sample_points, sample_labels):
        ...
        for x in range(len(sample_points)):
            dist = KNN_Classifier.euclidean_distance(self, point, sample_points[x])
            distances.append((dist, sample_labels[x]))
        distances.sort(key=lambda tup: tup[0])
        temp = []
        for x in distances:
            temp.append(x[1])
        returnMe = KNN_Classifier.pick_label(self, temp)
        return returnMe
        raise NotImplementedError