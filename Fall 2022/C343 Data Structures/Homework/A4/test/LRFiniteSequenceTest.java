 ...
 **/
    ...
    @Test
    void mapTests() {
        LRFiniteSequence<String> seq2 = new LRFiniteSequence<>(5);
        seq2.insertLeft("Hello");
        seq2.insertLeft("Fun");
        SmallStream<Boolean> boo = seq2.map(str -> str.length() > 4);
        LRFiniteSequence<Boolean> seq2ans = new LRFiniteSequence<>(5);
        seq2ans.insertLeft(true);
        seq2ans.insertLeft(false);
        Iterator<Boolean> seqIter = boo.iterator();
        Iterator<Boolean> resIter = seq2ans.iterator();
        while (seqIter.hasNext() && resIter.hasNext()) {
            assertEquals(seqIter.next(), resIter.next());
        }

        SmallStream<Integer> boo2 = seq2.map(str -> str.length());
        LRFiniteSequence<Integer> boo2ans = new LRFiniteSequence<>(5);
        boo2ans.insertLeft(5);
        boo2ans.insertLeft(3);
        Iterator<Integer> boo2Iter = boo2.iterator();
        Iterator<Integer> boo2ansIter = boo2ans.iterator();
        while (seqIter.hasNext() && resIter.hasNext()) {
            assertEquals(boo2ansIter.next(), boo2Iter.next());
        }
    }
    ...
    void minTypeTest() {
        //Check char
        LRFiniteSequence<Character> seq2 = new LRFiniteSequence<>(5);
        seq2.insertLeft('c');
        seq2.insertLeft('b');
        seq2.insertLeft('d');
        seq2.insertLeft('a');
        assertEquals(Optional.of('a'), seq2.min(Character::compare));
    }

    @Test
    void filterTypeTest() {
        //Check if empty gets added
        seq.insertLeft(5);
        seq.insertLeft(4);
        SmallStream<Integer> ss = seq.filter(n -> n % 2 == 0);
