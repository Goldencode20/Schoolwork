...
public class Trie implements WordCollection {
    ...
    void insert(String s) {
        if(s.length() != 0){
            if(s.length() == 1){
                ...
                children.get(s.charAt(0)).endsHere = true;
            } else {
                if(children.containsKey(s.charAt(0))){
                    children.get(s.charAt(0)).insert(s.substring(1));
                } else {
                    Trie newTrie = new Trie();
                    newTrie.insert(s.substring(1));
                    children.put(s.charAt(0), newTrie);
                }
            }
        }
    }
     ...
     */
    boolean search (String s, boolean fullWord) { //TODO fullword check at the start
        if(s.length() == 0){
            return true;
        }
        if (s.length() == 1) {
            if (children.containsKey(s.charAt(0))) {
                if (fullWord) {
                    return children.get(s.charAt(0)).endsHere;
                } else {
                    return true;
                }
            } else {
